...
#	mv $PROJECT_ROOT/appdir/$PROJECT_NAME*  $PROJECT_ROOT/backup/$PROJECT_NAME-$(date +%Y-%m-%d--%H-%M).jar
	...
	cp  -f $PACKAGE_NAME  $PROJECT_ROOT/appdir/
	echo 	" "
        echo "## 打包完成 ##"     
	echo 	" "
}
...
if [ "$*x" != "x" ]
  then
    echo "传参举例，参数为 '-n project_name -r project_root -p package_name' "
    PART_T ${PART_LIST}
  else
    ...
    if [ "${PROJECT_NAME}x" == "x" ] && [ "${PROJECT_ROOT}x" == "x" ] && [ "${PACKAGE_NAME}x" == "x" ]
      then
        echo "请正确传递参数,正确参数为 '-n project_name -r project_root -p package_name' "
        exit
    fi
fi

echo " PROJECT_NAME=$PROJECT_NAME ,  PROJECT_ROOT=$PROJECT_ROOT , PACKAGE_NAME=$PACKAGE_NAME "
