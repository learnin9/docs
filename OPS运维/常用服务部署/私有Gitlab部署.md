为了方便，直接在CentOS7下进行部署

1. 安装基础的依赖关系

```
yum install vim lsof iotop nload curl net-tools policycoreutils-python policycoreutils openssh-server openssl-clients -y
```

2. 安装gitlab软件,在[清华开源镜像站上下载](https://mirrors.tuna.tsinghua.edu.cn/gitlab-ce/yum/el7)最新的版本进行安装

```
yum install gitlab-ce-10.5.2-ce.0.el7.x86_64.rpm -y
```
...
```
openssl genrsa -des3 -out gitlab.key    
openssl req -new -key gitlab.key -out gitlab.csr
cp gitlab.key gitlab.key.org
openssl rsa -in gitlab.key.org -out gitlab.key
#发送到CA中心
scp -r gitlab.csr root@172.19.0.10:/tmp/ca/
```

* CA中心执行

```
openssl ca -keyfile /etc/pki/CA/private/cakey.pem -cert /etc/pki/CA/cacert.pem -in gitlab.csr -out gitlab.crt
#发送回证书
scp -r nginx.crt root@172.19.0.12:/etc/gitlab/ssl/
```
...
gitlab_rails['smtp_address'] = "192.168.1.1"
...
external_url 'https://git.cloud.org'
